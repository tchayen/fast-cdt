...
var map: SelectedMap = .playground;
...
fn run() !void {
    ...
    switch (map) {
        .playground => try presets.playground(storage),
        .point_removal => try presets.pointRemoval(storage),
        .self_intersecting => try presets.selfIntersecting(storage),
        .grid => try presets.grid(storage),
        .tiny_square => try presets.tinySquare(storage),
    }
}
