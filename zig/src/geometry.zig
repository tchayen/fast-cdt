...
pub fn getIntersecting(queue: *Queue, seed: *HalfEdge, e1: Point, e2: Point) !void {
    ...
    std.log.debug("start_edge = {any}", .{start_edge.?});
...
pub fn collectBoundary(edges: *EdgeContext, boundary: *Ring, p: Point) !void {
    ...
    std.log.debug("starting search in {any}", .{start});
    ...
    while (to_destroy.pop()) |edge| {
        if (edge.twin) |_| {
            std.log.debug("destroying {any}", .{edge});
            edges.destroy(edge);
        }
    }
}
...
pub fn removeCollinear(edges: *EdgeContext, boundary: *Ring) !void {
    ...
    while (true) {
        ...
        if (is_on_boundary and collinear) {
            std.log.debug("Removing collinear {any}", .{b.value.origin});
...
pub fn validate(edges: *EdgeContext) !void {
    ...
    while (it.next()) |e| {
        if (e.next.?.next.?.next.? != e) {
            std.log.debug("VALIDATION: {any} does not form △!", .{e});
            success = false;
        }
        if (e.twin != null and e.twin.?.twin != e) {
            std.log.debug("VALIDATION: {any} has a twin but that twin has different twin!", .{e});
            success = false;
        }
        if (e.twin != null and e.fixed and !e.twin.?.fixed) {
            std.log.debug("VALIDATION: {any} is fixed but twin is not", .{e.origin});
        }
    }
